...
@ConditionalOnClass(name = "org.springframework.web.bind.MissingServletRequestParameterException")
public class MissingServletRequestParameterExceptionHandler extends AbstractExceptionHandler {
    ...
    public ResponseEntity<?> handle(
        ...
        final ResponseEntity<ApiErrorResponse> response = ResponseEntity.status(statusCode).body(new ApiErrorResponse(
                statusCode,
                singleton(type),
                singletonList(String.format("%s: parameter is missing", exception.getParameterName())),
                LocalDateTime.now()
        ));
